...
public class Config {
            ...
            try(FileWriter writer = new FileWriter(configFile)) {
                for (Map.Entry<String, Object> property : properties.entrySet()) {
                    if (property.getValue() instanceof JsonElement) {
                        ...
                        MyCommands.LOGGER.error("Unable to parse json config property! File: " + name + ".json with Property: " + property.getKey());
                        continue;
                    }
                }
    ...
    private void loadConfig() {
        if(!configFile.exists()) {
            MyCommands.LOGGER.error("Unable to load config file " + name + ".json, File not found!");
            return;
        }

        try {
            MyCommands.LOGGER.info("Loading config file " + name + ".json");
    ...
    private void saveConfig(FileWriter writer) {
        MyCommands.LOGGER.info("Creating config file " + name + ".json");
